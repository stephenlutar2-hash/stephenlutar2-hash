...
export default function Login() {
  ...
  const [entraConfig, setEntraConfig] = useState<{ configured: boolean; clientId?: string; authority?: string; scopes?: string[] } | null>(null);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundImage: "radial-gradient(ellipse at center, rgba(6,182,212,0.06) 0%, transparent 70%)" }}>
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-display font-bold tracking-wider">VESSELS</h1>
          <p className="text-muted-foreground mt-2 text-sm tracking-widest uppercase">Maritime Intelligence Platform</p>
        </div>

        <div className="glass-panel rounded-2xl p-8 space-y-6 border-cyan-500/10">
          {error && (
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder="Enter username" className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition text-sm" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter password" className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition text-sm" />
              </div>
            </div>
            <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-cyan-500 to-emerald-500 text-white font-bold py-3 rounded-lg tracking-wider uppercase text-sm hover:opacity-90 transition disabled:opacity-50">
              {loading ? "Authenticating..." : "Access Vessels"}
            </button>
          </form>
          ...
          <button
            onClick={() => {
              localStorage.setItem("szl_token", "demo_token_vessels");
              localStorage.setItem("szl_user", "demo");
              localStorage.setItem("szl_demo_mode", "true"); trackEvent("demo", "enter", "vessels");
              setLocation("/dashboard");
            }}
          ...
          <p className="text-center text-muted-foreground/60 text-xs">No credentials needed — browse with sample data</p>
        </div>
      </div>
    </div>
  );
