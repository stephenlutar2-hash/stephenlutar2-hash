...
import AlertTriageQueue from "@/components/AlertTriageQueue";
...
type DashView = "overview" | "triage" | "timeline" | "threats" | "incidents" | "scans" | "monitoring";

export default function Dashboard() {
  const [threats, setThreats] = useState<Threat[]>([]);
  const [incidents, setIncidents] = useState<Incident[]>([]);
  const [scans, setScans] = useState<ScanRecord[]>([]);
  const [tab, setTab] = useState<DashView>("overview");
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [monitoring, setMonitoring] = useState<any>(null);
  const [, setLocation] = useLocation();
  const [selectedEntity, setSelectedEntity] = useState<{ entity: string; type: string } | null>(null);
  const [socStage, setSocStage] = useState("triage");
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      <header className="border-b border-white/10 bg-black/50 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-violet-600 flex items-center justify-center shadow-[0_0_15px_rgba(6,182,212,0.3)]"><Shield className="w-4 h-4 sm:w-5 sm:h-5" /></div>
            <div>
      ...
      <div className="border-b border-cyan-500/10 bg-cyan-500/[0.02] px-4 sm:px-6 py-1.5">
        <div className="max-w-7xl mx-auto flex items-center justify-between text-[10px] font-mono">
          ...
          <div className="hidden sm:flex items-center gap-3 text-gray-500">
            <span>Uptime 99.97%</span><span>·</span><span>Last sync 2 min ago</span><span>·</span><span className="text-amber-400">Demo Mode</span>
          </div>
        </div>
      </div>
      ...
      <PullToRefresh onRefresh={async () => { fetchData(); fetchMonitoring(); }} className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 pb-20 md:pb-8">
        ...
        <SocWorkflow activeStage={socStage} onStageChange={setSocStage} />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-6">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              className={`bg-gradient-to-br ${stat.bg} to-transparent border border-white/10 rounded-2xl p-4 sm:p-5 backdrop-blur-sm hover:scale-[1.02] transition-transform cursor-pointer shadow-[inset_0_0_20px_rgba(255,255,255,0.02)]`}
            >
        ...
        <div className="flex gap-2 mt-6 mb-4 overflow-x-auto pb-1">
          {(["overview", "triage", "timeline", "threats", "incidents", "scans", "monitoring"] as DashView[]).map(t => (
        ...
        <AnimatePresence mode="wait">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      { label: "Threat Intel", desc: "Real-time feed", icon: ShieldAlert, color: "from-red-500/20 to-orange-500/20", link: "/threat-intel" },
                    ].map((card, i) => (
          ...
          {tab === "triage" && (
            <motion.div key="triage" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className={selectedEntity ? "lg:col-span-2" : "lg:col-span-3"}>
                  <AlertTriageQueue onEntityClick={(entity, type) => setSelectedEntity({ entity, type })} />
                </div>
                {selectedEntity && (
                  <EntityInvestigation
          ...
          {tab === "threats" && (
            <motion.div key="threats" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
          ...
          {tab === "incidents" && (
            <motion.div key="incidents" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
              <div className="space-y-3">
                {incidents.length === 0 ? (
                  <div className="bg-white/5 border border-white/10 rounded-2xl py-12 text-center text-gray-500">
                  ...
                  <motion.div key={inc.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5 hover:bg-white/[0.07] transition backdrop-blur-sm"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-3"><SeverityBadge severity={inc.severity} /><h3 className="font-bold text-sm">{inc.title}</h3></div>
          ...
          {tab === "scans" && (
            <motion.div key="scans" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {scans.length === 0 ? (
                  <div className="col-span-full bg-white/5 border border-white/10 rounded-2xl py-12 text-center text-gray-500">
                    <Scan className="w-10 h-10 mx-auto mb-3 opacity-20" />
                    <p className="text-sm">No scan records found.</p>
                  </div>
                ) : scans.map(scan => (
                  <motion.div key={scan.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                    className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm hover:border-white/20 transition-all"
                  >
                    <div className="flex items-center justify-between mb-3"><h3 className="font-bold text-sm">{scan.platform}</h3><StatusBadge status={scan.status} /></div>
                    <p className="text-gray-400 text-xs mb-4">{scan.scanType}</p>
                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div className="bg-white/5 rounded-lg p-2"><p className="text-lg font-black text-red-400">{scan.threatsFound}</p><p className="text-xs text-gray-500">Found</p></div>
                      <div className="bg-white/5 rounded-lg p-2"><p className="text-lg font-black text-emerald-400">{scan.threatsBlocked}</p><p className="text-xs text-gray-500">Blocked</p></div>
                      <div className="bg-white/5 rounded-lg p-2"><p className="text-lg font-black text-cyan-400">{scan.duration > 0 ? `${(scan.duration / 60).toFixed(1)}m` : "—"}</p><p className="text-xs text-gray-500">Duration</p></div>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          {tab === "monitoring" && (
            <motion.div key="monitoring" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm">
                  <div className="flex items-center gap-2 mb-4"><Server className="w-5 h-5 text-cyan-400" /><h3 className="font-bold text-sm uppercase tracking-wider">Server Status</h3></div>
                  {monitoring?.server ? (
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm">
                  <div className="flex items-center gap-2 mb-4"><Cpu className="w-5 h-5 text-violet-400" /><h3 className="font-bold text-sm uppercase tracking-wider">Memory</h3></div>
                  {monitoring?.server?.memoryUsageMB ? (
                    <div className="space-y-3 text-sm">
                      ...
                      <div className="mt-2">
                        ...
                        <div className="w-full bg-white/5 rounded-full h-2"><div className="bg-gradient-to-r from-cyan-500 to-violet-500 h-2 rounded-full transition-all" style={{ width: `${Math.round((monitoring.server.memoryUsageMB.heapUsed / monitoring.server.memoryUsageMB.heapTotal) * 100)}%` }} /></div>
                      </div>
                    </div>
                  ) : <p className="text-gray-500 text-sm">Loading...</p>}
                </div>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm">
                  <div className="flex items-center gap-2 mb-4"><Shield className="w-5 h-5 text-cyan-400" /><h3 className="font-bold text-sm uppercase tracking-wider">Identity Provider</h3></div>
                  {monitoring?.identity ? (
                    <div className="space-y-3 text-sm">
