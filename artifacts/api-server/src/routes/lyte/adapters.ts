...
export class AdapterOrchestrator {
  private telemetry = new TelemetryAdapter();
  private security = new SecurityFeedAdapter();
  private logistics = new LogisticsAdapter();
  private projectStatus = new ProjectStatusAdapter();
  ...
  async getAllSignals(filters?: SignalFilters): Promise<{ signals: Signal[]; total: number; mode: string }> {
    ...
    if (filters) {
      if (filters.domain) allSignals = allSignals.filter(s => s.domain === filters.domain);
      if (filters.severity) allSignals = allSignals.filter(s => s.severity === filters.severity);
      if (filters.freshness) allSignals = allSignals.filter(s => s.freshness === filters.freshness);
      if (filters.status) allSignals = allSignals.filter(s => s.status === filters.status);
      if (filters.source) allSignals = allSignals.filter(s => s.source.toLowerCase().includes(filters.source!.toLowerCase()));
      if (filters.owner) allSignals = allSignals.filter(s => s.owner.toLowerCase().includes(filters.owner!.toLowerCase()));
    }
  ...
  async getDashboardSummary(): Promise<DashboardSummary> {
    ...
    return {
      ...
      deploymentCoverage: 78,
      dnsTlsCompletion: 89,
      attentionQueue: active
  ...
  getRecommendations(): { recommendations: Recommendation[]; total: number; mode: string } {
    const recs: Recommendation[] = [
      { id: "rec-002", title: "Optimize Firestorm API response times", reasoning: "Response times consistently above 200ms SLA. Root cause analysis points to unoptimized database queries in the simulation engine. Caching layer would reduce p95 latency by estimated 40%.", projectedImpact: "Restore SLA compliance; improve user experience for 2,400 daily sessions", priority: 1, actionType: "investigate", suggestedActions: ["Profile top 5 slowest API endpoints", "Implement Redis caching for simulation state", "Add query optimization for scenario builder"], relatedSignals: ["sig-001"], owner: "Backend Team" },
    ...
    return { recommendations: recs, total: recs.length, mode: resolveMode() };
  }
  ...
  getImpactMetrics(): { metrics: ImpactMetric[]; total: number; mode: string } {
    const metrics: ImpactMetric[] = [
      ...
      { id: "imp-002", title: "Latency Risk — Firestorm & Dreamscape", category: "risk", value: "4,200 sessions/day", trend: "up", narrative: "Combined latency issues across Firestorm (>200ms) and Dreamscape (>5s AI pipeline) causing measurable user drop-off. Estimated revenue impact: $32K/quarter.", relatedProjects: ["Firestorm", "Dreamscape"] },
      ...
      { id: "imp-006", title: "Security Posture Score", category: "efficiency", value: "94/100", trend: "up", narrative: "Portfolio-wide security score at 94/100. ROSIE threat engine, Aegis vulnerability scanning, and Firestorm simulation all contributing. Aegis compliance module nearing completion.", relatedProjects: ["ROSIE", "Aegis", "Firestorm"] },
    ];
